...
/**
 * Builds a YouTube-style prompt for static data analysis (title and description)
 * Returns description and categories like the YouTube analyzer
 */
export function buildStaticDataYouTubePrompt(
  categories: string[], 
  title: string, 
  description: string
): string {
  const customIntroduction = `אתה עוזר אנליזה של תוכן בתחום הפילוסופיה היהודית. התוכן הוא של פרופ׳ שלום צדיק. אני רוצה שבסיכום שלך תכתוב את הנושא המרכזי שפרופ׳ שלום מדבר עליו ותסביר אותו בקצרה.
...
עליך:
1. ליצור תיאור קצר וממוקד של התוכן בעברית שמתחיל במילים "בסרטון זה פרופ׳ שלום צדיק מדבר על" ואז ממשיך ב-4-5 משפטים בלבד שמסכמים את הנושא העיקרי שפרופ׳ שלום צדיק מדבר עליו
2. לזהות את הקטגוריות מהרשימה שתואמות בצורה הטובה ביותר לתוכן

עקרונות לבחירת קטגוריות:
1. דיוק אקדמי: בחר רק קטגוריות שהן רלוונטיות באופן ישיר לתוכן
2. זיהוי ראשי תיבות: רס״ג = רבי סעדיה גאון, רמב״ם = רבי משה בן מימון, וכו׳
3. זיהוי ספרים ומחבריהם:
   - מורה נבוכים, שמונה פרקים, משנה תורה = רמב״ם
   - אמונות ודעות = רס״ג
...
החזר תשובה בפורמט JSON בלבד עם המבנה הבא:
{
  "description": "בסרטון זה פרופ׳ שלום צדיק מדבר על [המשך עם 4-5 משפטים בעברית על הנושא המרכזי]",
  "categories": ["קטגוריה מהרשימה 1", "קטגוריה מהרשימה 2"]
}
...
אל תוסיף סימני קוד (\`\`\`) או כל עיצוב markdown אחר.
החזר JSON נקי לחלוטין.
ודא שהתיאור מתחיל במילים "בסרטון זה פרופ׳ שלום צדיק מדבר על" ומורכב מ-4 עד 5 משפטים בלבד.
ודא שאתה בוחר רק קטגוריות מהרשימה שסופקה.
זכור: דיוק אקדמי חשוב יותר מכמות הקטגוריות.
חשוב: אל תהסס לבחור קטגוריות שונות לחלוטין מהדוגמאות - כל תוכן הוא ייחודי ונתח אותו בנפרד בהתאם לתוכן הספציפי שלו.`;
